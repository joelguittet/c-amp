 ...
 */
int amp_push(amp_msg_t *amp, amp_type_e type, ...) {
  
  assert(NULL != amp);

  void *blob = NULL; size_t size = 0; char *str = NULL; int64_t bint = 0; cJSON *json = NULL;
...
int amp_encode(amp_msg_t *amp, void **buf, size_t *size) {
  ...
  unsigned char *tmp1; size_t position = 0;
  ...
  while (NULL != curr_field) {
    ...
    size_t len = 0; size_t curr_field_size = 0; void *data = NULL;
    ...
    /* Copy field data */
    memcpy(&tmp1[position], data, curr_field_size); position += curr_field_size;
...
int amp_decode(amp_msg_t *amp, void **buf, size_t *size) {
  ...
  /* Decoding completed */
  *buf = (void *)(((unsigned char *)*buf) + position); *size -= position;
